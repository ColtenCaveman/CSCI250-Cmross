# CS3-cmross


## My github repo for CSCI250

| Name | Value |
|:---|:---|
| **Course** | CSCI250 - CS3: Intro to algorithms |
| **Semester** | Spring 2026 |
| **Student** | Colten Ross |
| **Mav Username**            | cmross |
| **GitHub Username**         | ColtenCaveman |
| **Repository**          | https://github.com/ColtenCaveman/CSCI250-Cmross |

## Assignments

### Assignment #3

| Name | Value |
| :--- | :--- |
| Name | 1 -  |
| Description |  |
| Due Date |  |
| Status |  |
| Location |  |
| Self Grade |  |
| Notes |  |


### Assignment #2

| Name | Value |
| :--- | :--- |
| Name | 2 - Sorting Algorithms |
| Description | An Assignment to implement various different sorting algorithms in Python |
| Due Date | 5-14-2026 |
| Status | Complete |
| Location | https://github.com/ColtenCaveman/CSCI250-Cmross/tree/main/Assignments/Assignment%202 |
| Self Grade | 80 |
| Notes | completed, is pretty late though |


### Assignment #1

| Name | Value |
| :--- | :--- |
| Name | 1 - Big O notation |
| Description | a paper assignment to learn more about big O notation |
| Due Date | 5-14-2026 |
| Status | Complete |
| Location | D2L |
| Self Grade | 100 |
| Notes | N/A |
